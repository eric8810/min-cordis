...
async fn emit_async_safe() {
    ...
    impl Listener<Ping> for ValueL {
        fn call<'a>(
            ...
            _c: &'a Ctx,
            e: &'a Ping,
        ) -> min_cordis::BoxFuture<'a, Result<Option<u32>, CordisError>> {
            let seen = self.0.clone();
            let v = e.value;
            Box::pin(async move {
...
async fn prepend_order() {
    ...
    ctx.events()
        .on_opt(
            ...
            min_cordis::EventOptions { prepend: true },
        )
        .unwrap();
...
async fn single_service_evict() {
    ...
    let d1 = Arc::new(Mutex::new(None::<min_cordis::Disposer>));
    let d1c = d1.clone();
...
async fn evict_after_consumer_disposed_completes() {
    let ctx = Ctx::root().unwrap();
    let d1: Arc<Mutex<Option<min_cordis::Disposer>>> = Arc::new(Mutex::new(None));
    let dc = d1.clone();
    let provider = ctx.plugin(simple("P", move |ctx: &Ctx| {
        let dc = dc.clone();
        Box::pin(async move {
