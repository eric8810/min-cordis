...
//!
//! ```text
//! cargo run -p min-cordis-agent --example demo                       # deepseek(读 DEEPSEEK_API_KEY)
//! AIMUX_PROVIDER=ollama AIMUX_MODEL=qwen3:8b cargo run -p min-cordis-agent --example demo
//! ```
//!
...
async fn main() {
    ...
    loop {
        if rx.borrow().state == min_cordis::FiberState::Pending {
            break;
        }
