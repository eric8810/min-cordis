...
//!
//! ```text
//! cargo run -p min-cordis-agent --example tui                        # deepseek(读 DEEPSEEK_API_KEY)
//! AIMUX_PROVIDER=ollama AIMUX_MODEL=qwen3:8b cargo run -p min-cordis-agent --example tui
//! ```
//!
...
use min_cordis::Ctx;
use min_cordis_agent::{llm_key, AgentDriverPlugin, ToolDef, ToolsPlugin, TuiPlugin};
    ...
    let model = match aimux_providers::provider(&provider, None, &model_id, None) {
        ...
        Err(e) => {
            ...
            if provider == "deepseek" && std::env::var_os("DEEPSEEK_API_KEY").is_none() {
                eprintln!(
                    "hint: export DEEPSEEK_API_KEY=... ; or offline: cargo run -p min-cordis-agent --example tui_scripted"
                );
            }
            std::process::exit(1);
        }
