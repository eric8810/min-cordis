...
> 结论先行:**代码级热更**(dylib/热补丁)与**生命周期级热重载**(实例卸载/重装配)是两个正交问题;min-cordis v4 做的是后者,先例在 Erlang/OSGi 一系;前者(dyxlib/subsecond)是可组合的互补能力。
...
| min-cordis v4 对应 | **不做**(正交,可组合) | **M2 核心**(fiber 驱逐/重载/restart) |
...
- 与 min-cordis"清理期自访问仍有效"(provide 卸载三段序)同类问题:**换件瞬间,依赖旧件的代码怎么办**。
...
3. **正交组合点(写入 M4 候选)**:subsecond / hot-lib-reloader 负责"拿到新代码",min-cordis 负责"安全换件"——旧插件 dispose → 依赖者驱逐 → 新插件装配 → 依赖者重载。代码级工具不进核心,不做耦合。
