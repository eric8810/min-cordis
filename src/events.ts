import { defineProperty } from './utils.ts'
import type { Promisify } from './utils.ts'
import { Context } from './context.ts'
import { Fiber, FiberState } from './fiber.ts'
import { DisposableList, isObject, symbols } from './utils.ts'
 ...
 * all listeners together, `serial` awaits listeners in order until one bails,
...
declare module './context.ts' {
  export interface Context {
    ...
    /**
     * Dispatch an event synchronously. Rejections from async listeners are
     * routed to `ctx.logger.error` instead of escaping as unhandled
     * rejections; synchronous listener errors still propagate to the caller.
     *
     * @param name — the event name.
     * @param args — arguments passed to every listener.
     */
...
export class EventsService {
  // Null-prototype map: `__proto__`/`constructor`/Object.prototype member
  // names must resolve to nothing instead of colliding with inherited
  // properties (audit: registering or dispatching such names crashed).
  ...
  /**
   * Run listeners synchronously. Rejections from async listeners are caught
   * and logged rather than escaping as unhandled rejections.
   *
   * @param args — optional `this`, the event name, then listener arguments.
   */
  emit(...args: any[]) {
    for (const cb of this.dispatch('emit', args)) {
      const result = cb(...args)
      if (isObject(result) && typeof (result as PromiseLike<any>).catch === 'function') {
        (result as Promise<any>).catch(error => this.ctx.logger.error(error))
      }
    }
  }
