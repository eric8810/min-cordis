...
import type { Dict } from './utils.ts'
...
type GetPluginConfig<P> = P['Config'] extends StandardSchemaV1<any, infer T> ? T : GetPluginParameters<P>[0]
...
declare module './context.ts' {
  export interface Context {
     ...
     *
     * Returns the fiber itself (made thenable via a non-enumerable `then`),
     * so `restart()`/`update()` calls through the handle mutate the real
     * lifecycle state. `await` resolves to the same fiber.
     *
...
export class RegistryService {
  ...
  plugin(plugin: Plugin, config?: any, getOuterStack = buildOuterStack()) {
    ...
    // Prototype wrapper (not a copy): `restart()`/`update()` re-anchor to
    // `this.ctx.fiber` before mutating lifecycle state, so calls through the
    // wrapper drive the real fiber (upstream cordis 752dbee semantics). The
    // wrapper exists so `await` can resolve to the real fiber without
    // thenable recursion.
